      ...
      "desiredState": {
         "manifest": {
           ...
           "containers": [{
             ...
             "ports": [{"containerPort": 80, "hostPort": 9998}]
           }]
         }
       },
...
To play with the service itself, find the name of a frontend, grab the external IP of that host from the [Google Cloud Console][cloud-console] or the `gcutil` tool, and visit `http://<host-ip>:9998`.

```shell
$ gcutil listinstances
```

You may need to open the firewall for port 9998 using the [console][cloud-console] or the `gcutil` tool. The following command will allow traffic from any source to instances tagged `kubernetes-minion`:

```shell
$ gcutil addfirewall --allowed=tcp:9998 --target_tags=kubernetes-minion kubernetes-minion-9998
```

If you are running Kubernetes locally, you can just visit http://localhost:9998
